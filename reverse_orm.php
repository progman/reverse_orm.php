...
*/
	...
	$rc = add_dir($target_dir.'/'.$server_name.'/'.$table_schema.'.'.$table_name.'/lck'); // delete
	if ($rc->is_ok() === false) return $rc;

	$rc = del_dir($target_dir.'/'.$server_name.'/'.$table_schema.'.'.$table_name.'/lck'.'/auto');
	if ($rc->is_ok() === false) return $rc;

	$rc = add_dir($target_dir.'/'.$server_name.'/'.$table_schema.'.'.$table_name.'/lck'.'/auto');
	if ($rc->is_ok() === false) return $rc;

	$rc = add_dir($target_dir.'/'.$server_name.'/'.$table_schema.'.'.$table_name.'/lck'.'/hand');
	if ($rc->is_ok() === false) return $rc;
...
function generate_lock($arg)
{
	...
	$tmp .= '// PLEASE DO NOT EDIT IT !!! THIS FILE IS GENERATED BY https://github.com/progman/reverse_orm.php'."\n";
	$tmp .= make_line();
	$tmp .= 'function sql_orm_lck__'.$table_schema.'__'.$table_name.'__0001($sql_handle)'."\n";
	$tmp .= '{'."\n";
	$tmp .= '	$result = new result_t(__FUNCTION__, __FILE__);'."\n";
	$tmp .= '//	$item = null;'."\n";
	$tmp .= ''."\n";
	...
	$tmp .= '	$sql_tag = "SQLXXX".__FUNCTION__;'."\n";
	$tmp .= '	$sql_str = "'."\n";
	$tmp .= 'LOCK TABLE '.$table_schema.'.'.$table_name.' IN ACCESS EXCLUSIVE MODE; -- ".$sql_tag."\n";'."\n";
	...
	$filename = $target_dir.'/'.$server_name.'/'.$table_schema.'.'.$table_name.'/lck/auto/0001.php';
//echo '['.$filename.']'."\n";
	$rc = libcore__file_set($filename, $tmp);
	if ($rc === false)
	{
...
//	$tmp .= "\n";
	...
	$tmp .= "\n";
	$tmp .= "\n";
	$tmp .= "// lck ".$table_name." (until end of transaction)\n";
	$tmp .= '	$rc = $'.$table_schema.'__'.$table_name.'->lck();'."\n";
	$tmp .= '	if ($rc->is_ok() === false) return $rc;'."\n";
...
//		}
	...
	$tmp .= "			[ \"name\"=> \"ORM_LCK_0001\", \"type\"=> \"LCK\", \"func\"=> \"sql_orm_lck__".$table_schema."__".$table_name."__0001\", \"filter_list\"=> [], \"sort_list\"=> [] ]\n";
